...
func (m *Intake2) validateBusinessCase(formats strfmt.Registry) error {
	...
	for i := 0; i < len(m.BusinessCase); i++ {
		if swag.IsZero(m.BusinessCase[i]) { // not required
			continue
		}

		if m.BusinessCase[i] != nil {
			if err := m.BusinessCase[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("BusinessCase" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
